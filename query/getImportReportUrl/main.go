...
func HandleRequest(ctx context.Context, req request) (*response, error) {
	...
	out, err := importSvc.PresignReportGetURL(ctx, tenantID, jobID)
	if err != nil {
		return nil, err
	}
	return &response{URL: out.URL, ExpiresInSeconds: out.ExpiresInSeconds}, nil
}

func main() {
	logger.Info("booting_get_import_report_url", map[string]any{"localstack_hostname": os.Getenv("LOCALSTACK_HOSTNAME")})
	defer func() {
		if r := recover(); r != nil {
